...
These numbers come from *[NYC open data](link) and ARTICLE C*.
```{r}
...
my_severity_by_age =
  ...
  rename(., prob_need_bed = percent_symptomatic_requiring_hospital) %>% 
  mutate(., prob_die_in_hospital = 0.5*percent_hospital_cases_requiring_critical_care) %>% 
  select(., -percent_hospital_cases_requiring_critical_care, -infection_fatality_ratio)
